import numpy as np
from sobel_experiment import grad_magnitude
import cv2
import glob
...
outer_region_right = [
    {'start_region':((996, 387),(1275, 658)),
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.1, math.pi/2)
     },
    {'start_region':((974, 361),(1276, 538)),
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.0, math.pi/2)
     },
    {'start_region':((809, 409),(1269, 717)), #liberal picks up many lines, but will need some filter/weight policy
     'end_region': ((0, 0),(1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.0, math.pi/2)
     }
...
inner_region_left = [
    {'start_region':((9, 488),(314, 608)),  # picks up lines on outer left, but can keep
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'negative',
     'slope_bounds': (0.2, math.pi/2)
     },
    {'start_region':((9, 488),(314, 608)),  # note the secondary lowering of threshold to ensure values
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'negative',
     'slope_bounds': (0.1, math.pi/2)
     },
...
inner_region_right = [
    {'start_region':((730, 485),(1105, 702)),  # picks up lines on outer left, failes on one image
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.15, math.pi/2)
     },
    {'start_region':((730, 485),(1105, 702)),  # note the secondary lowering of threshold to ensure values
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.1, math.pi/2)
     },
    {'start_region':((730, 485),(1105, 702)),  # note the secondary see if it causes problems
     'end_region': ((0, 0), (1280, 720)),
     'slope_sign':'positive',
     'slope_bounds': (0.0, math.pi/2)
     }
...
def find_filtered_candidate_lines(img):
    ...
    for bounding_line_set, name in zip(highly_probable_bounding_lines, probable_names):
        for top_priority in bounding_line_set:
            ...
            if len(temp_holder_lines)>0: ## or maybe considier appending and some weight scheme for which comes first??
...
def extrapolate_to_vanishing(lines, y_region=(350, 450), threshold=5, x_region=(400,800)):
    # we are interested  in y's that fall within a range,
    # and we bucket the x's
...
def find_hull(lines):
    # find largest polygon which describes parallel lanes in image
    lines = lines.reshape((-1,4))
    ...
    slopes = slopes[finite_slopes]  # finite slopes should have finite intercepts
    ...
    right_slope = weighted_avg3(slopes[right_lines],
                                length[right_lines],
    ...
    ## calculate the four points that are the hull

def transform_coordinates(img, hull, x_bound=(200, 1200)):
    slopes, intercepts = get_slopes_intercepts(np.array(hull).reshape(-1,4))
    xs = project_y_to_x([600], slopes, intercepts)
    x_shift = (xs[1]/2 + xs[0]) - 700
    ...
    if find_regions_interest:
        ...
        for img_path in straight_images + curved_images:
            ...
            if see_all_lines:
                ...
                lines = cv2.HoughLinesP(region_interest, 1, math.pi / 180, 20, np.array([]), minLineLength=20,
                                        maxLineGap=100)
                filtered_lines = filter_lines(lines, **inner_region_right[filter_priority]).reshape((-1,1,4))
                blank_lines_img = np.zeros((region_interest.shape[0], region_interest.shape[1], 3))
                draw_lines(blank_lines_img, filtered_lines)
                cv2.namedWindow('lines')
                cv2.imshow('lines', blank_lines_img)
                ...
                print('extrapolation)')
                best_lines = find_filtered_candidate_lines(img)
                ...
                filtered_line_image = weighted_img(np.uint8(blank), np.uint8(img))
                ...
                convergent_lines = extrapolate_to_vanishing(best_lines_accum, y_region=(350, 450),
                                                           threshold=10, x_region=(500,600))

                projected_lines = collect_filter_projections(img, best_lines_accum[convergent_lines], np.array([400]))
                ...
                #original = img.copy()
                #unfiltered_projected_lines = collect_filter_projections(img, best_lines_accum, np.array([400]))
                #draw_lines(blank2, np.int32(unfiltered_projected_lines).reshape((-1, 1, 4)))

                #filtered_line_image = weighted_img(np.uint8(blank), np.uint8(img))
                #unfiltered_line_image = weighted_img(np.uint8(blank2), np.uint8(original))


                convergent_lines_hull = find_hull(best_lines_accum[convergent_lines])

                blank = np.zeros_like(img)
                draw_lines(blank, np.int32(convergent_lines_hull).reshape((-1, 1, 4)))


                convergent_line_image = weighted_img(np.uint8(blank), np.uint8(img))

                f, (ax1, ax2) = plt.subplots(1, 2, figsize=(24,9))
                f.tight_layout()
                ax1.imshow(cv2.cvtColor(convergent_line_image,cv2.COLOR_BGR2RGB))
                ax2.imshow(cv2.cvtColor(transform_coordinates(img, convergent_lines_hull), cv2.COLOR_BGR2RGB))
                ...
                #take best_lines, extrapolate to 'vanishing points'
                # throw out bad lines
                # pick out bounding areas/ polygon
